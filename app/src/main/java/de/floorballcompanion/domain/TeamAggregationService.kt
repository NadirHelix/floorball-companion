...
class TeamAggregationService @Inject constructor(
     ...
     * Strategy:
     * 1. Check cache first
     * 2. Fetch all leagues across all Verbände via leagues.json (single call)
     * 3. For each league-modus league: check table for teamId
     * 4. For cup/playoff leagues (no table): check schedule
    ...
    suspend fun discoverLeaguesForTeam(
        ...
        val cupLeagues = allLeagues.filter { it.leagueModus != "league" }

        leagueModusLeagues.forEachIndexed { index, league ->
            onProgress(index + 1, allLeagues.size)
            if (league.id == knownLeagueId) return@forEachIndexed

            try {
                val table = api.getTable(league.id)
                if (table.any { it.teamId == teamId }) {
                    discoveredMappings.add(
                        TeamLeagueMapping(
                            teamId = teamId,
                            leagueId = league.id,
        ...
        // Phase 2: check schedules for cup/playoff leagues
        val leagueModusOffset = leagueModusLeagues.size
        cupLeagues.forEachIndexed { index, league ->
            onProgress(leagueModusOffset + index + 1, allLeagues.size)
            if (league.id == knownLeagueId) return@forEachIndexed
